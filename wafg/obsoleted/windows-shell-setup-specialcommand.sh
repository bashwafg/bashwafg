windows-shell-setup-specialcommand() {
    if [[ "$1" =~ ^[0-9]+$ ]]; then
        local index="$1" command desc req order
        eval "command=\${specialcommandarr_${index}_commandline}"
        eval "desc=\${specialcommandarr_${index}_description}"
        eval "req=\${specialcommandarr_${index}_requiresinput}"
        eval "order=\${specialcommandarr_${index}_order}"
